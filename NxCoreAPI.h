...
#define NxCoreAPI_h
////////////////////////////////////////////////////////////////////////////////////////////////////////
//	CONTENTS COPYRIGHT 2003-2021(c) Nanex, LLC, Winnetka, IL. ALL RIGHTS RESERVED.					  //
...
typedef struct {
	NxMutable int			UserData1;	// first  32-bit user data space. Valid for
	NxMutable int			UserData2;	// second 32-bit user data space.
...
// when NxCoreSystem.Status == NxCORESTATUS_ERROR
#define NxCSERROR_CANCEL					1  // User cancel;
#define NxCSERROR_ERROR						14 // Error processing tape
...
// used to Filter opra exchanges by channel(s)
#define NxOPRAID_SELECT_CHAN    0x00400000      // EXCLUDES EVERYTHING but ONLY selected opra channels - OR'd with specific (OPRAID_) channels desired
#define NxOPRAID_ONLY           0x00800000      // includes ONLY the Opra exchanges and EXCLUDES everything else
#define NxOPRAID_BBO_01		    0x00000001
#define NxOPRAID_BBO_02		    0x00000002
#define NxOPRAID_BBO_03		    0x00000004
#define NxOPRAID_BBO_04		    0x00000008
#define NxOPRAID_FULL_01		0x00000010
#define NxOPRAID_FULL_02		0x00000020
#define NxOPRAID_FULL_03		0x00000040
#define NxOPRAID_FULL_04		0x00000080
#define NxOPRAID_FULL_05		0x00000100
#define NxOPRAID_FULL_06		0x00000200
#define NxOPRAID_FULL_07		0x00000400
#define NxOPRAID_FULL_08		0x00000800
#define NxOPRAID_FULL_09		0x00001000
#define NxOPRAID_FULL_10		0x00002000
#define NxOPRAID_FULL_11		0x00004000
#define NxOPRAID_FULL_12		0x00008000
#define NxOPRAID_FULL_13		0x00010000
#define NxOPRAID_FULL_14		0x00020000
#define NxOPRAID_FULL_15		0x00040000
#define NxOPRAID_FULL_16		0x00080000
// special case to REMOVE a specific PermID... combine with PermID to remove
#define NxPERMID_REMOVE         0x00100000
...
struct NxCoreSystem { // 76/104 bytes
	NxMutable int	UserData;				// UserData value passed to NxCoreProcessTape
	...
	struct NxAccessStatus*	pNxAStatus;				// Ver 2.0 and higher (NxCoreAccess and NxCoreAPI.dll): during real-time processing, status of NxCoreAccess. Updated once per second.
};
...
struct NxCoreExgQuote {// 88/88 bytes.
	struct NxCoreQuote		coreQuote;// 40/40 bytes.
	int				BestAskPrice;
	int				BestBidPrice;
...
struct NxCoreMMQuote {// 48/56 bytes
	struct NxCoreQuote			coreQuote;						//40 bytes.
...
struct NxCoreTrade {  // 88/88 bytes.
	...
	struct NxCTAnalysis			nxAnalysis;		// 24/24 bytes
...
struct NxCoreSymbolChange { // 16/32 bytes
	...
	struct NxOptionHdr*	pnxOptionHdrOld;  // non-zero for option contract changes
...
union NxCoreData {	// 88/88 bytes
	struct NxCoreExgQuote		ExgQuote;		// 88/88 bytes
	struct NxCoreMMQuote		MMQuote;		// 48/56 bytes
	struct NxCoreTrade			Trade;			// 88/88 bytes
	struct NxCoreCategory		Category;		// 12/24 bytes
	struct NxCoreSymbolChange	SymbolChange;	// 16/32 bytes
	struct NxCoreSymbolSpin	SymbolSpin;		// 20/20 bytes
};
...
struct NxCoreHeader {// 48/56 bytes
	...
	struct NxOptionHdr*	pnxOptionHdr;
...
struct NxCoreMessage { //152/156 bytes
	struct NxCoreHeader			coreHeader;
	union NxCoreData				coreData;
	unsigned int			MessageType;
	unsigned char			alignment[4];		// 2.3.198, 2007.04.16
	struct NxCore3Ext*				pnxCore3Ext;		// 2015.11.11
};
...
struct NxCoreStateExgQuotes { // 248/488 bytes
	...
	unsigned short		BestAskExg;
	unsigned short		BestBidExg;
	struct NxCoreStateExgQuote StateExgQuotes[24]; // 20/20 bytes
};
...
struct NxCoreStateMMQuotes {  // 12292/16392 bytes
	...
	unsigned char		PriceType;
	unsigned char		NasdaqBidTick;		  // for nasdaq stocks. nasdaqBidTick.
	struct NxCoreStateMMQuote  StateMMQuotes[512];	  // 24/32 bytes
};
...
/////////////////////////////////////////////////////////////////////
typedef int(__stdcall *NxCoreCallback)		  (const struct NxCoreSystem* pNxCoreSys, const struct NxCoreMessage* pNxCoreMsg);
typedef int(__stdcall *NxCoreCallbackTapeList) (void* pYourParam, const struct NxCoreTapeFile* pNcTF);
typedef int(__stdcall *NxCoreCallbackAPIList)  (void* pYourParam, const struct NxCoreAPIDLLFile* pNcTF);
...
typedef int(__stdcall *NxCoreListTapesFolder)   (const char* folderName, NxCoreCallbackTapeList stdcallback, void* pYourParam);
typedef int(__stdcall *NxCoreListAPIDLLs)	    (unsigned int controlFlags, NxCoreCallbackAPIList stdcallback, void* pUserParam);
typedef int(__stdcall *NxCoreListAPIDLLsFolder) (const char* folderName, NxCoreCallbackAPIList stdcallback, void* pUserParam);
...
/////////////////////////////////////////////////////////////////////
typedef int(__stdcall *NxCoreStateGetLastTrade)(struct NxCoreStateTrade*	  pStateTrade, NxString* pnxsSymOrCtc/*=0*/);
typedef int(__stdcall *NxCoreStateGetExgQuotes)(struct NxCoreStateExgQuotes*  pStateExgQuotes, NxString* pnxsSymOrCtc/*=0*/);
typedef int(__stdcall *NxCoreStateGetMMQuotes) (unsigned short ReportingExg, struct NxCoreStateMMQuotes* pStateMMQuotes, NxString* pnxsSymOrCtc/*=0*/);
...
typedef int(__stdcall *NxCorePriceFormat)		  (char* szBuff, int lPrice, unsigned char PriceType, int expandWidth/*=0*/, int bCommas/*=false*/);
...
typedef int(__stdcall *NxCoreSignalWaitEvent)			  (const struct NxCoreSystem* pNxCoreSystem, int bAlwaysSignal);
...
#define cszNxCoreListTapesFolder            "sNxCoreListTapesFolder"
#define cszNxCoreListAPIDLLs				"sNxCoreListAPIDLLs"
#define cszNxCoreListAPIDLLsFolder			"sNxCoreListAPIDLLsFolder"
